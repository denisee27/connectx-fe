...
export const Schedule = () => {
    ...
    return (
        <div className="min-h-screen mx-auto max-w-7xl bg-white px-4 sm:px-8 text-foreground">
            {/* Header */}
            ...
            <div className="mt-8 relative">
                ...
                <div className="absolute left-[132px] sm:left-[178px] top-2 bottom-0 w-px bg-border" aria-hidden />
                {/* Animated list container */}
                <div
                    key={activeTab}
                    className="space-y-10 transition-all duration-300 ease-out"
                >
                    {data.map((day, di) => (
                        <div key={`${day.dayLabel}-${di}`} className="relative">
                            {/* Layout: 1) Date/Day, 2) Line & Dot, 3) Cards */}
                            <div className="grid grid-cols-[100px_24px_1fr] sm:grid-cols-[140px_28px_1fr] gap-5 sm:gap-6 items-start">
                                {/* 1. Date & Day */}
                                ...
                                <div className="relative">
                                    <div className="absolute left-1/2 -translate-x-1/2 top-2 h-3 w-3 rounded-full bg-accent border-2 border-white shadow" aria-hidden />
                                </div>
...
const EventCard = ({ time, title, location, guests, thumbnail, onClick }) => (
    <div className="flex items-center justify-between gap-4 sm:gap-6 rounded-2xl border border-border bg-card p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex-1 min-w-0">
            <div className="text-sm text-muted-foreground">{time}</div>
            <h3 className="mt-1 text-lg sm:text-xl font-semibold leading-snug line-clamp-2">
                {title}
            </h3>
            ...
            <button
                onClick={onClick}
                type="button"
                className="hover:cursor-pointer mt-4 inline-flex items-center gap-2 rounded-full bg-accent px-4 py-2 font-medium text-black hover:bg-primary transition-colors"
            >
                Detail Event <ChevronRight size={16} />
            </button>
        </div>
